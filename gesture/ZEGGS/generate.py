...
def generate_gesture(
    ...
    # Load Networks
    network_speech_encoder = torch.load(path_network_speech_encoder_weights,map_location=torch.device('cpu')).to(device)
    network_speech_encoder.eval()

    network_decoder = torch.load(path_network_decoder_weights,map_location=torch.device('cpu')).to(device)
    network_decoder.eval()

    network_style_encoder = torch.load(path_network_style_encoder_weights,map_location=torch.device('cpu')).to(device)
